...
class LLMService(LoggerMixin):
    ...
    async def expand_topic(self, topic: str, language: str = "zh") -> ExtendedTopic:
        ...
        try:
            ...
            # 解析响应
            expanded_topic = self._parse_topic_expansion_response(response, topic)
            
            self.logger.info(f"主题扩展完成，生成 {len(expanded_topic.extended_keywords)} 个关键词")
            return expanded_topic
            
        except Exception as e:
            self.logger.error(f"主题扩展失败: {e}")
            # 返回默认的扩展结果
    ...
    async def generate_awesome_list(
        ...
        try:
            prompt = self._build_awesome_list_prompt(topic, search_results, language)
...
"""
            ...
            try:
                ...
                keywords = [word.strip().strip('"\'') for word in response.split('\n') if word.strip()]
    ...
    def _parse_topic_expansion_response(self, response: str, original_topic: str) -> ExtendedTopic:
        ...
        try:
            ...
            for line in lines:
                ...
                elif "搜索查询" in line or "Search Queries" in line:
            ...
            # 确保包含原始主题
            if original_topic not in extended_keywords:
                extended_keywords.insert(0, original_topic)
            
            # 如果解析失败，提供默认值
            if not extended_keywords:
                extended_keywords = [original_topic]
            if not search_queries:
                search_queries = [original_topic, f"{original_topic} tutorial"]
            ...
            self.logger.warning(f"解析主题扩展响应失败: {e}")
            return ExtendedTopic(
                original_topic=original_topic,
                extended_keywords=[original_topic],
                related_concepts=[],
                search_queries=[original_topic]
            )
